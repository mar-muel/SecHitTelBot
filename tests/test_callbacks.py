...
class TestPresentActionAllBranches:
    ...
    async def test_present_action_calls_end_game_when_over(self, bot, session5):
        ...
        # Session should be removed from controller.games
        assert session5.cid not in controller.games
...
class TestNominateCallback:
    ...
    async def test_nominate_steps_engine_to_vote(self, bot, session5):
        ...
        context.bot = bot
        await controller.nominate_chosen_chancellor(update, context)
...
class TestChooseKillCallback:
    ...
    async def test_kill_non_hitler(self, bot, session5):
        ...
        context.bot = bot
        bot.reset_mock()
        await controller.choose_kill(update, context)
...
class TestChooseInspectCallback:
    ...
    async def test_inspect_reveals_party(self, bot, session5):
        ...
        context.bot = bot
        bot.reset_mock()
        await controller.choose_inspect(update, context)
...
class TestChooseChooseCallback:
    ...
    async def test_special_election(self, bot, session5):
        ...
        context.bot = bot
        bot.reset_mock()
        await controller.choose_choose(update, context)
